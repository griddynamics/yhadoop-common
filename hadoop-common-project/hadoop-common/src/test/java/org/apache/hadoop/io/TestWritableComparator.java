...
public class TestWritableComparator {
  ...
  /**
   * test {@code WritableComparator.compare()} method
   */
  @Test
  public void testWritableComparator() {
    ...
    assertTrue("testCompareTest less error",
        wCmp.compare(tbyte1, 0, tbyte1.length, tbyte2, 0, tbyte2.length) < 0);
    assertTrue("testCompareTest more error",
        wCmp.compare(tbyte2, 0, tbyte2.length, tbyte1, 0, tbyte1.length) > 0);
    assertTrue("testCompareTest equal error",
        wCmp.compare(tbyte1, 0, tbyte1.length, tbyte1, 0, tbyte1.length) == 0);
  }

  /**
   * test static methods
   * {@code WritableComparator readUnsignedShort()/readVInt()/readVLong} with
   * different parameters
   */
  @Test
  public void testReadTypes() {
    try {
      int unShort = WritableComparator.readUnsignedShort(
          new byte[] { 0, -128 }, 0);
      ...
      unShort = WritableComparator.readUnsignedShort(new byte[] { 0, 0, 0, 0,
          -128 }, 3);
      assertEquals("readUnsignedShort error", unShort, 128);

      int intValue = WritableComparator.readVInt(new byte[] { 1, 0, 0, 0, 0, 0,
          0, 0 }, 0);
      assertTrue("readVInt error", intValue == 1);

      long longValue = WritableComparator.readVLong(new byte[] { 1, 0, 0, 0, 0,
          0, 0, 0 }, 0);
