...
public class TestUserGroupInformationWithTicketCache {
  ...
  public void testGetBestUGIFromTicketCache() throws Exception {
    ...
    final UserGroupInformation ugi = 
        UserGroupInformation.loginUserFromKeytabAndReturnUGI(fullPrincipalName, ktbFilePath);
    
    assertNotNull(ugi);
    assertEquals(AuthenticationMethod.KERBEROS, ugi.getAuthenticationMethod());
    assertEquals(AuthenticationMethod.KERBEROS, UserGroupInformation.getRealAuthenticationMethod(ugi));
    // for branch-0.23 we definitely login from the keytab:
