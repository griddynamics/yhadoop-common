...
public class TestFileSink {
  ...
  private static final int SNAPSHOT_PERIOD_SEC = 5; // seconds
  ...
  public void testFileSink() throws Exception {
    ...
    new ConfigBuilder().add("*.period", SNAPSHOT_PERIOD_SEC)
    ...
    // NB: wait snapshot period + 50%:
    Thread.sleep(SNAPSHOT_PERIOD_SEC * 1500L);
    
    ms.stop();
    ms.shutdown();
    
    final InputStream is = new FileInputStream(outFile);
    final ByteArrayOutputStream baos = new ByteArrayOutputStream((int)outFile.length());
    IOUtils.copyBytes(is, baos, 1024, true/*close*/);
    final String outFileContent = new String(baos.toByteArray(), "UTF-8");
