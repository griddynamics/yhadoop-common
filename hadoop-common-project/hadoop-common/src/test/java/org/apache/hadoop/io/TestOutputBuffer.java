...
public class TestOutputBuffer {
  ...
  public void testOutputBufferWithResize() throws IOException {
    ...
    while (size < out.getLength()) {
      Assert.assertTrue(
          "TestOutputBuffer testOutputBufferWithResize error !!!",
          out.getData()[size] == bytes[size]);
      size++;
    }
    out.close();
  }
  ...
  /**
   * test {@code OutputBuffer} reset() method
   */
  @Test
  public void testOutputBufferReset() throws IOException {
    ...
    out.write(in, bytes.length);
    out.reset();
    Assert.assertTrue("TestOutputBuffer testOutputBufferReset error !!!",
        out.getLength() == 0);
    out.close();
  }
  ...
  static final class Bytes {
    ...
    private static final Random rnd = new Random(123456l);
