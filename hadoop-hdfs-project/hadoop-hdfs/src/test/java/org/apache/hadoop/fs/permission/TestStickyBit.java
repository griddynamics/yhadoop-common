...
public class TestStickyBit {
  ...
  public void testStickyBitPersistence() throws IOException {
    ...
    try {
      Configuration conf = new HdfsConfiguration();
      conf.setBoolean(DFSConfigKeys.DFS_PERMISSIONS_ENABLED_KEY, true);
      cluster = new MiniDFSCluster.Builder(conf).baseDfsDir(baseDir).numDataNodes(4).build();
      FileSystem hdfs = cluster.getFileSystem();
      ...
      cluster = new MiniDFSCluster.Builder(conf).baseDfsDir(baseDir).numDataNodes(4).format(false).build();
